...
$(document).ready(async () => {
  try {
    const ids = [2, 12, 22, 32, 42];
    const postPromises = ids.map((id) => getObject(`${domain}/posts/${id}`));
    const postsData = await Promise.all(postPromises);
    
    const userPromises = postsData.map((post) => getObject(`${domain}/users/${post.userId}`));
